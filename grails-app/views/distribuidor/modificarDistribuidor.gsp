...
<html>
	<head>
		...
		<title><g:message code="Modificar Distribuidora" args="[entityName]" /></title>
	</head>
	<body>
			...
			<table>
				...
				<g:each in="${distribuidorInstanceList}" status="i" var="distribuidorInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="edit" id="${distribuidorInstance.id}">${fieldValue(bean: distribuidorInstance, field: "nombre")}</g:link></td>
